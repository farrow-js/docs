...
Router factory for `farrow-http`. It extends from `Pipeline` in [farrow-pipeline](./01-farrow-pipeline.mdx). So there are all api in `Pipeline`.
...
:::info
learn more about Schema Builder from [farrow-schema](../schema/01-farrow-schema.mdx).
:::
